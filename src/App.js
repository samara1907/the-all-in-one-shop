import React, {createRef} from 'react';
import './App.css';
import Home from './components/Home/Home';
import {
  BrowserRouter as Router ,
 Route, Switch, Link
} from "react-router-dom";
import Shop from './components/Shop/Shop';
import About from './components/About/About';
import Contact from './components/Contact/Contact';
import Login from './components/Login/Login';
import Register from './components/Register/Register';
import { Component } from 'react';

class App extends Component {
  constructor(props) {
    super(props);
    this.wrapper = React.createRef();
  }
  render(){
    return (
      <Router ref={this.wrapper}>
      <div className="App">
      <div>
            <header className="bg-white black-80 tc pv4 avenir">
                <Link to={"/"} className="bg-black-80 ba b--black dib pa3 w2 h2 br-100">
                    <svg className="white" data-icon="skull" viewBox="0 0 32 32" style={{fill:'currentcolor'}}><title>skull icon</title><path d="M16 0 C6 0 2 4 2 14 L2 22 L6 24 L6 30 L26 30 L26 24 L30 22 L30 14 C30 4 26 0 16 0 M9 12 A4.5 4.5 0 0 1 9 21 A4.5 4.5 0 0 1 9 12 M23 12 A4.5 4.5 0 0 1 23 21 A4.5 4.5 0 0 1 23 12"></path></svg>
                    </Link>
                    <h1 className="mt2 mb0 baskerville i fw1 f1">The All In One Shop</h1>
                    <h2 className="mt2 mb0 f6 fw4 ttu tracked">Where you can find all your needs</h2>
                    <nav className="bt bb tc mw7 center mt4">
                    <Link to={"/"} className="f6 f5-l link bg-animate black-80 hover-bg-lightest-blue dib pa3 ph4-l" >Home</Link>
                    <Link to={"shop"} className="f6 f5-l link bg-animate black-80 hover-bg-light-blue dib pa3 ph4-l" >Shop</Link> 
                    <Link to={"about"} className="f6 f5-l link bg-animate black-80 hover-bg-light-pink dib pa3 ph4-l" >About</Link>
                    <Link to={"contact"} className="f6 f5-l link bg-animate black-80 hover-bg-light-yellow dib pa3 ph4-l">Contact</Link>
                    <Link to={"login"} className="f6 f5-l link bg-animate black-80 hover-bg-light-red dib pa3 ph4-l">Login</Link>
                    <Link to={"register"} className="f6 f5-l link bg-animate black-80 hover-bg-light-gray dib pa3 ph4-l">Signup</Link>
                </nav>
                </header>

        </div>
        <Switch>
            <Route path="/" exact component={Home} />
            <Route path="/shop" component={Shop}/>
            <Route path="/about" component={About}/>
            <Route path="/contact" component={Contact}/>
            <Route path="/login" component={Login}/>
            <Route path="/register" component={Register}/>
            </Switch>
      </div>
      </Router>
    );
  }
  
}

export default App;
